...
export type DesignerState = {
  ...
  // ── viewport actions ──
  setView: (zoom: number, pan: ScreenPoint) => void;
  setPan: (pan: ScreenPoint) => void;
  panBy: (dx: number, dy: number) => void;
  zoomIn: () => void;
  zoomOut: () => void;
  zoomAround: (factor: number, anchor: ScreenPoint) => void;
...
export function createDesignerStore(initial: FormSchema): StoreApi<DesignerState> {
  return createStore<DesignerState>((set, get) => {
    ...
    return {
      ...
      selection: [],
      zoom: 1,
      pan: { x: PAGE_MARGIN, y: PAGE_MARGIN },
      ...
      zoomAround: (factor, anchor) => {
        ...
        if (next === zoom) return;
        // Keep the canvas point under `anchor` fixed on screen.
        const cx = (anchor.x - pan.x) / zoom;
        const cy = (anchor.y - pan.y) / zoom;
        set({
          zoom: next,
          pan: { x: anchor.x - cx * next, y: anchor.y - cy * next },
        });
      },

      zoomIn: () => {
        const { viewport } = get();
        get().zoomAround(ZOOM_STEP, {
          x: viewport.width / 2,
          y: viewport.height / 2,
        });
      },
      ...
      setZoom: (zoom) => {
        const { viewport } = get();
        const factor = clampZoom(zoom) / get().zoom;
        get().zoomAround(factor, {
          x: viewport.width / 2,
          y: viewport.height / 2,
        });
      },
      ...
      fitToScreen: () => {
        const { schema, viewport } = get();
        if (viewport.width === 0 || viewport.height === 0) return;
        const bounds =
          contentBounds(firstPage(schema).elements) ?? {
            x: 0,
            y: 0,
            width: schema.canvas?.width ?? DEFAULT_CANVAS_WIDTH,
            height: schema.canvas?.height ?? 720,
        ...
        const zoom = clampZoom(
          Math.min(
            (viewport.width - pad) / Math.max(1, bounds.width),
            (viewport.height - pad) / Math.max(1, bounds.height),
          ),
        );
        set({
          zoom,
      ...
      resetView: () => {
        ...
        set({
          zoom: 1,
          pan: {
            x: Math.max(PAGE_MARGIN, (viewport.width - canvasWidth) / 2),
            y: PAGE_MARGIN,
          },
        });
      },
