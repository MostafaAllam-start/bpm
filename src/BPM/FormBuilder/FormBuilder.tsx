...
import { isFormSchema, type FormSchema } from "./types";
...
import {
  DesignerModeContext,
  type DesignerMode,
} from "./designer/DesignerModeContext";
...
export type FormBuilderProps = {
  ...
  // Designer mode: 'form' (default) shows all fields + palette + property panel;
  // 'email' and 'pdf' show only display fields with inline editing.
  type?: DesignerMode;
};
...
export default function FormBuilder({
  ...
  maximized,
  onToggleMaximize,
  type: designerType = "form",
}: FormBuilderProps) {
  const isDocMode = designerType === "email" || designerType === "pdf";
  ...
  const initial = useMemo<FormSchema>(
    () =>
      existingSchema && isFormSchema(existingSchema)
        ? existingSchema
        : buildInitialSchema(t, actorId, actorLabel, designerType),
  ...
  function handleNew(): void {
    model.load(buildInitialSchema(t, actorId, actorLabel, designerType));
    setSavedFormData(null);
    setError(null);
    setTab("design");
  }
  ...
  async function handleSubmit(): Promise<void> {
    ...
    try {
      ...
      const savedSchema = isDocMode ? { ...schema, submittable: false } : schema;
      setSavedFormData(savedSchema);
      setError(null);
      onSave?.(savedSchema, labelInput.trim());
  ...
  // In email/pdf mode hide the Logic tab (no conditions on display-only
  // templates) but keep Preview so the rendered output can be inspected.
  const tabs: { id: TabId; label: string }[] = isDocMode
    ? [
  ...
  return (
    <div className="form-editor">
      ...
      <DesignerModeContext.Provider value={designerType}>
          ...
          <div className="dz-body">
            {tab === "design" && (
              ...
              >
                {showFields && (
                  <Palette
                    onAdd={(type, title) => model.addField(type, title)}
                  />
                )}
                ...
                schema={
                  isDocMode
                    ? {
                        ...
                        submittable: false,
                        pages: model.schema.pages.map((p) => ({
                          ...p,
                          elements: p.elements.filter((f) => {
                            const def = getFieldType(f.type);
                            return def?.group === "display" && f.type !== "group";
                          }),
                        })),
                      }
                    : model.schema
      ...
      {showTemplates && (
        <TemplateGallery
          templates={TEMPLATES.filter(
            (tpl) => (tpl.mode ?? "form") === designerType,
          )}
          onSelect={(schema) => {
            model.load(schema);
            setTab("design");
