    ...
    const interactable = interact(el)
      .draggable({
        ...
        listeners: {
          ...
          move(event) {
            const st = store.getState();
            // In PDF mode the page is scaled via CSS transform; divide client-pixel
            // deltas by the scale so 1 pointer pixel maps to 1/scale canvas pixel.
            const scale =
              parseFloat(
                (el.closest("[data-canvas-scale]") as HTMLElement | null)
      ...
      .resizable({
        ...
        listeners: {
          ...
          move(event) {
            ...
            running = {
              ...running,
              x: running.x + (d?.left ?? 0) / scale,
              // Skip vertical repositioning when autoHeight is on.
              y: autoH ? running.y : running.y + (d?.top ?? 0) / scale,
              width: running.width + (d?.width ?? 0) / scale,
              // Skip height changes when autoHeight is on.
              height: autoH ? running.height : running.height + (d?.height ?? 0) / scale,
            };
