// The free-form design canvas. A scrolling "viewport" element hosts a "stage"
// that centres the canvas "page" (the form's design surface); inside the page
// sit one LayoutContainer per field, all in canvas-pixel space. The form renders
// at a fixed 1:1 scale — when the page is larger than the viewport the viewport
// scrolls (there is no zoom or pan). The viewport owns the interactions the
// individual widgets don't: marquee selection, palette drops, and the keyboard
// shortcuts (undo/redo, delete, duplicate, nudge…).

import {
  useCallback,
  useEffect,
  useLayoutEffect,
  useRef,
  useState,
  type DragEvent,
  type PointerEvent as ReactPointerEvent,
} from "react";
import { useTranslation } from "react-i18next";

import type { FieldType, LayoutBox } from "../types";
import type { VariableRef } from "@shared/variables.ts";
import { getFieldType } from "../utils/fieldTypes";
import { useDesignerMode } from "./DesignerModeContext";
import { resolveText } from "../utils/text";
import {
  SUBMIT_NAME,
  TITLE_NAME,
  useDesigner,
  useDesignerStoreApi,
} from "./designerStore";
import { LayoutContainer } from "./LayoutContainer";
import { SubmitContainer } from "./SubmitContainer";
import { TitleContainer } from "./TitleContainer";
import { ADD_FIELD_MIME } from "./Palette";
import TextFormatToolbar from "./TextFormatToolbar";
import {
  boxesBottom,
  clampColumns,
  fieldsInBox,
  groupHeaderHeight,
  GROUP_PAD,
  PAGE_PADDING,
  rectFromPoints,
  rowBands,
} from "./canvasLayout";
import { breakpointWidth, resolveLayout, type Positioned } from "../utils/responsive";

type CanvasRendererProps = { locale: string; variables?: VariableRef[] };

// Did a pointer press land on the bare canvas (viewport / stage / page / empty
// hint) rather than on a field widget? Widget presses must be left to the
// widget's own selection handler and interact.js. The widget body and the empty
// hint are `pointer-events: none`, so a press over them resolves to the page;
// the widget root (.dz-lc) is the target for presses on a widget.
function isCanvasBackground(el: EventTarget | null): boolean {
  const node = el as HTMLElement | null;
  if (!node || !node.classList) return false;
  return (
    node.classList.contains("dz-canvas2") ||
    node.classList.contains("dz-stage") ||
    node.classList.contains("dz-page") ||
    node.classList.contains("dz-page-empty")
  );
}

// Is the user currently typing in a form control (so global keyboard shortcuts
// must stand down)?
function isEditingTarget(el: EventTarget | null): boolean {
  const node = el as HTMLElement | null;
  if (!node) return false;
  const tag = node.tagName;
  return (
    tag === "INPUT" ||
    tag === "TEXTAREA" ||
    tag === "SELECT" ||
    node.isContentEditable
  );
}

export default function CanvasRenderer({ locale, variables }: CanvasRendererProps) {
  const { t, i18n } = useTranslation("form");
  const isRTL = i18n.dir(locale) === "rtl";
  const store = useDesignerStoreApi();
  const mode = useDesignerMode();
  const isDocMode = mode === "email" || mode === "pdf";

  const allPages = useDesigner((s) => s.schema.pages);
  const activePageIndex = useDesigner((s) => s.activePageIndex);
  const allElements = useDesigner((s) => s.schema.pages[s.activePageIndex]?.elements ?? []);
  // In email/pdf mode only display fields (no inputs/choices) are shown.
  const elements = isDocMode
    ? allElements.filter((f) => getFieldType(f.type)?.group === "display")
    : allElements;
  const submit = useDesigner((s) => s.schema.submit);
  const submittable = useDesigner((s) => s.schema.submittable);
  const titleBox = useDesigner((s) => s.schema.titleBox);
  const titleText = useDesigner((s) => s.schema.title);
  const selection = useDesigner((s) => s.selection);
  const snap = useDesigner((s) => s.snap);
  const canvas = useDesigner((s) => s.schema.canvas);
  const viewport = useDesigner((s) => s.viewport);
  // Whether the column grid / row band guideline overlays are shown (toolbar
  // toggles). Row guides also appear transiently while dragging regardless.
  const showColGuides = useDesigner((s) => s.showColGuides);
  const showRowGuides = useDesigner((s) => s.showRowGuides);
  const alignGuides = useDesigner((s) => s.alignGuides);
  // The breakpoint the canvas is designing; fields render at their resolved
  // layout for it and the page takes that breakpoint's width.
  const activeBreakpoint = useDesigner((s) => s.activeBreakpoint);

  const viewportRef = useRef<HTMLDivElement>(null);
  const pageRef = useRef<HTMLDivElement>(null);

  // Marquee rectangle (canvas space) while drag-selecting; null when inactive.
  const [marquee, setMarquee] = useState<{
    x: number;
    y: number;
    width: number;
    height: number;
  } | null>(null);

  // Inline editing (email/pdf mode): which field is being edited in-place.
  const [editingName, setEditingName] = useState<string | null>(null);

  const selectionSet = new Set(selection);
  const primaryName = selection.length ? selection[selection.length - 1] : null;

  // Convert a client point to canvas (page) space. Field coordinates live in the
  // page's own pixel space (its top-left is the origin), and the canvas renders
  // 1:1, so this is just the offset from the page element's box — scrolling and
  // centring are handled by the layout, not by arithmetic here.
  const toCanvas = useCallback((clientX: number, clientY: number) => {
    const rect = pageRef.current?.getBoundingClientRect();
    const left = rect?.left ?? 0;
    const top = rect?.top ?? 0;
    return { x: clientX - left, y: clientY - top };
  }, []);

  // ── viewport size → store; auto-fit the form's design width to the container ─
  useLayoutEffect(() => {
    const el = viewportRef.current;
    if (!el) return;
    let lastW = -1;

    // An auto-width form fills the container width at 1:1 (a small margin each
    // side); the layout centres the page and scrolls when it overflows.
    const fit = (force: boolean) => {
      const w = el.clientWidth;
      const st = store.getState();
      st.setViewportSize(w, el.clientHeight);
      if (w <= 0) return;
      // The form's design width auto-fits the container on every breakpoint (not
      // just "All"), so toggling the field / properties panels reflows the form to
      // the new width. Only re-fit when the width actually changed; `force` (mount
      // passes) ignores that so the initial render always fits.
      if (!force && Math.abs(w - lastW) <= 1) return;
      lastW = w;
      if (st.schema.canvas?.autoWidth !== false) {
        st.fitCanvasToWidth(Math.max(360, w));
      }
    };

    // Fit pre-paint, then again on the next frame to catch layout that only
    // settles after mount (the properties panel, web fonts, the modal's open
    // transition) — this is what makes the *initial* render fill the container.
    fit(true);
    const raf = requestAnimationFrame(() => fit(true));
    const ro = new ResizeObserver(() => fit(false));
    ro.observe(el);
    return () => {
      cancelAnimationFrame(raf);
      ro.disconnect();
    };
  }, [store, activeBreakpoint]);

  // ── global keyboard shortcuts (scoped to while the canvas is mounted) ─────
  useEffect(() => {
    const onKeyDown = (e: KeyboardEvent) => {
      if (isEditingTarget(e.target) || isEditingTarget(document.activeElement)) {
        return;
      }
      const st = store.getState();
      const mod = e.ctrlKey || e.metaKey;

      if (mod && e.key.toLowerCase() === "z") {
        e.preventDefault();
        if (e.shiftKey) st.redo();
        else st.undo();
        return;
      }
      if (mod && e.key.toLowerCase() === "y") {
        e.preventDefault();
        st.redo();
        return;
      }
      if (mod && e.key.toLowerCase() === "a") {
        e.preventDefault();
        st.selectAll();
        return;
      }
      if (mod && e.key.toLowerCase() === "d") {
        e.preventDefault();
        st.duplicateSelected();
        return;
      }
      if (e.key === "Delete" || e.key === "Backspace") {
        if (st.selection.length) {
          e.preventDefault();
          st.deleteSelected();
        }
        return;
      }
      if (e.key === "Escape") {
        st.clearSelection();
        return;
      }
      if (e.key.startsWith("Arrow") && st.selection.length) {
        e.preventDefault();
        const base = st.snap ? st.gridSize : 1;
        const step = e.shiftKey ? base * 5 : base;
        const dx = e.key === "ArrowLeft" ? -step : e.key === "ArrowRight" ? step : 0;
        const dy = e.key === "ArrowUp" ? -step : e.key === "ArrowDown" ? step : 0;
        st.nudgeSelected(dx, dy);
      }
    };
    window.addEventListener("keydown", onKeyDown);
    return () => {
      window.removeEventListener("keydown", onKeyDown);
    };
  }, [store]);

  // ── background pointer: marquee-select ────────────────────────────────────
  const marqueeStart = useRef<{ x: number; y: number; additive: boolean } | null>(
    null,
  );

  const onBackgroundPointerDown = (e: ReactPointerEvent<HTMLDivElement>) => {
    // This handler sits on the viewport, an ancestor of every widget, so a press
    // on a widget bubbles here too. We must NOT marquee/clear in that case — the
    // widget's own handler + interact.js own the gesture. Widgets no longer
    // stopPropagation (that would starve interact's document listener), so we
    // gate on the event target: act only when the press lands on bare canvas.
    if (!isCanvasBackground(e.target)) return;
    if (e.button !== 0) return;
    const start = toCanvas(e.clientX, e.clientY);
    marqueeStart.current = {
      x: start.x,
      y: start.y,
      additive: e.shiftKey || e.ctrlKey || e.metaKey,
    };
    if (!marqueeStart.current.additive) store.getState().clearSelection();
    setMarquee({ x: start.x, y: start.y, width: 0, height: 0 });
    viewportRef.current?.setPointerCapture(e.pointerId);
  };

  const onBackgroundPointerMove = (e: ReactPointerEvent<HTMLDivElement>) => {
    if (marqueeStart.current) {
      const s = marqueeStart.current;
      const cur = toCanvas(e.clientX, e.clientY);
      setMarquee(rectFromPoints(s.x, s.y, cur.x, cur.y));
    }
  };

  const endBackgroundPointer = (e: ReactPointerEvent<HTMLDivElement>) => {
    if (marqueeStart.current) {
      const s = marqueeStart.current;
      const cur = toCanvas(e.clientX, e.clientY);
      const rect = rectFromPoints(s.x, s.y, cur.x, cur.y);
      // A negligible drag is a click on empty space → just clears (already did).
      if (rect.width > 3 || rect.height > 3) {
        store.getState().selectInRect(rect, s.additive);
      }
      marqueeStart.current = null;
      setMarquee(null);
      viewportRef.current?.releasePointerCapture(e.pointerId);
    }
  };

  // ── palette drop → add a field at the drop point ──────────────────────────
  const onDragOver = (e: DragEvent) => {
    if (e.dataTransfer.types.includes(ADD_FIELD_MIME)) {
      e.preventDefault();
      e.dataTransfer.dropEffect = "copy";
    }
  };
  const onDrop = (e: DragEvent) => {
    const type = e.dataTransfer.getData(ADD_FIELD_MIME);
    if (!type) return;
    const def = getFieldType(type as FieldType);
    // In email/pdf mode only display-type fields are allowed on the canvas.
    if (isDocMode && def?.group !== "display") return;
    e.preventDefault();
    // PDF mode has a multi-page layout; pageRef is attached to the single-page
    // canvas only. Find the active page element directly for correct coords.
    let at: { x: number; y: number };
    if (mode === "pdf") {
      const activePageEl = viewportRef.current?.querySelector(
        `[data-page-index="${activePageIndex}"].is-active`,
      ) as HTMLElement | null;
      if (!activePageEl) return;
      const rect = activePageEl.getBoundingClientRect();
      // The page is CSS-scaled; divide by pdfScale to get canvas-pixel coords.
      at = { x: (e.clientX - rect.left) / pdfScale, y: (e.clientY - rect.top) / pdfScale };
    } else {
      at = toCanvas(e.clientX, e.clientY);
    }
    store
      .getState()
      .addField(
        type as FieldType,
        def ? t(`designer.types.${def.labelKey}`) : type,
        at,
      );
  };

  // Resolve each item's layout for the active breakpoint (base, or an override
  // that cascades to it). The canvas renders and measures these.
  const layoutFor = useCallback(
    (item: Positioned | undefined) => resolveLayout(item, activeBreakpoint),
    [activeBreakpoint],
  );
  const baseWidth = canvas?.width ?? 960;
  // The form keeps a single design width across every breakpoint: switching the
  // active breakpoint changes only which per-breakpoint layout overrides are
  // edited, never the canvas/page width. The column grid + guides follow it.
  const targetWidth = baseWidth;
  // PDF mode: fixed page dimensions (A4 by default).
  const pdfPageWidth = canvas?.pageWidth ?? 794;
  const pdfPageHeight = canvas?.pageHeight ?? 1123;
  // Scale factor to fit the PDF page inside the canvas container. The side ruler
  // (22 px) and a small right margin (32 px) are reserved so the page never
  // overflows the viewport. Clamped to ≤ 1 so we never upscale.
  const PDF_RULER_W = 22;
  const pdfScale =
    mode === "pdf" && viewport.width > PDF_RULER_W + 32
      ? Math.min(1, (viewport.width - PDF_RULER_W - 32) / pdfPageWidth)
      : 1;
  const pdfScaledW = Math.round(pdfPageWidth * pdfScale);
  const pdfScaledH = Math.round(pdfPageHeight * pdfScale);
  const submitLayout = layoutFor(submit);
  const titleLayout = layoutFor(titleBox);
  const resolvedBoxes = [
    ...elements.map((f) => layoutFor(f)),
    submitLayout,
    titleLayout,
  ].filter(Boolean) as LayoutBox[];
  // The page fills the container's height (so the design surface doesn't float in
  // empty canvas) but still grows past it to contain all content (fields + submit
  // button + title) when the form is taller than the viewport. The viewport-fill
  // term is computed from the live viewport size, which is set in the layout
  // effect before paint, so the surface is filled on the very first render.
  const contentBottom = boxesBottom(resolvedBoxes) + PAGE_PADDING;
  // The page is at least the breakpoint's target width, but grows to contain any
  // content that spills past it — e.g. a wider base ("All") layout inherited at a
  // narrower breakpoint — so the inherited design is always fully on the page
  // (not rendered off its right edge) and stays editable.
  const contentRight =
    resolvedBoxes.reduce((m, b) => Math.max(m, b.x + b.width), 0) + PAGE_PADDING;
  const pageWidth = Math.max(targetWidth, contentRight);
  const fillHeight = viewport.height;
  const pageHeight = Math.max(240, Math.round(contentBottom), Math.round(fillHeight));

  // Row band guide lines: a line at the top of every row. Shown when the
  // row-guides toggle is on.
  const rowGuides = showRowGuides
    ? rowBands(
        [
          ...elements.map((f) => layoutFor(f)),
          submitLayout,
        ].filter(Boolean) as LayoutBox[],
      ).map((b) => b.top)
    : [];

  // Column guidelines: the form's grid drawn across the page's content area for
  // the active breakpoint, plus — for any section that's selected or holds the
  // selection — that section's own column grid across its inner box. This is the
  // visual reference for sizing field widths in columns. Gated on the toggle.
  const columns = clampColumns(canvas?.columns);
  const resolvedEls = elements.map((f) => ({ ...f, layout: layoutFor(f) }));
  const groupGuides = showColGuides
    ? resolvedEls
        .filter((g) => g.type === "group" && g.layout)
        .filter(
          (g) =>
            selectionSet.has(g.name) ||
            fieldsInBox(resolvedEls, g.layout!, g.name).some((n) =>
              selectionSet.has(n),
            ),
        )
        .map((g) => {
          const headerH = groupHeaderHeight(g);
          return {
            name: g.name,
            left: g.layout!.x + GROUP_PAD,
            top: g.layout!.y + headerH,
            width: g.layout!.width - GROUP_PAD * 2,
            height: g.layout!.height - headerH - GROUP_PAD,
          };
        })
    : [];

  // PDF mode: render all pages stacked vertically, scaled to fit the container.
  if (mode === "pdf") {
    return (
      <div
        ref={viewportRef}
        className="dz-canvas2 dz-canvas2-pdf"
        onPointerDown={onBackgroundPointerDown}
        onPointerMove={onBackgroundPointerMove}
        onPointerUp={endBackgroundPointer}
        onPointerCancel={endBackgroundPointer}
        onDragOver={onDragOver}
        onDrop={onDrop}
      >
        <TextFormatToolbar />
        <div className="dz-stage dz-stage-pdf">
          {allPages.map((page, pageIdx) => {
            const isActive = pageIdx === activePageIndex;
            const pageEls = page.elements.filter(
              (f) => getFieldType(f.type)?.group === "display",
            );
            // Row guides for the active page when toggled on.
            const pageRowGuides =
              isActive && showRowGuides
                ? rowBands(
                    pageEls
                      .map((f) => layoutFor(f))
                      .filter(Boolean) as LayoutBox[],
                  ).map((b) => b.top)
                : [];

            return (
              <div key={pageIdx} className="dz-pdf-page-wrapper">
                <div className="dz-pdf-page-label">
                  {t("designer.page", { number: pageIdx + 1 })}
                </div>

                {/* Body row: side ruler + (top ruler + page) */}
                <div className={`dz-pdf-body${isRTL ? " is-rtl" : ""}`}>
                  {/* Vertical ruler (left in LTR, right in RTL) */}
                  <div
                    className="dz-pdf-dim-v"
                    style={{ height: pdfScaledH }}
                    aria-hidden="true"
                  >
                    <span className="dz-pdf-dim-v-label">{pdfPageHeight}px</span>
                  </div>

                  {/* Column: top ruler + scaled page */}
                  <div className="dz-pdf-col">
                    {/* Horizontal ruler */}
                    <div
                      className="dz-pdf-dim-h"
                      style={{ width: pdfScaledW }}
                      aria-hidden="true"
                    >
                      <span className="dz-pdf-dim-h-label">{pdfPageWidth}px</span>
                    </div>

                    {/* Scale container — fixes layout height to the visual page height */}
                    <div
                      className="dz-pdf-scale-wrap"
                      style={{ width: pdfScaledW, height: pdfScaledH }}
                    >
                      <div
                        data-page-index={pageIdx}
                        data-canvas-scale={pdfScale}
                        className={`dz-page dz-pdf-page${isActive ? " is-active" : ""}${snap ? " has-grid" : ""}`}
                        style={{
                          width: pdfPageWidth,
                          height: pdfPageHeight,
                          backgroundSize: `${store.getState().gridSize}px ${store.getState().gridSize}px`,
                          transform: `scale(${pdfScale})`,
                          transformOrigin: "top left",
                          position: "absolute",
                          top: 0,
                          left: 0,
                        }}
                        onClick={
                          !isActive
                            ? () => store.getState().setActivePageIndex(pageIdx)
                            : undefined
                        }
                      >
                        {pageEls.length === 0 && isActive && (
                          <p className="dz-page-empty">{t("designer.emptyHint")}</p>
                        )}

                        {/* Column guides for the active page */}
                        {showColGuides && isActive && (
                          <ColumnGuides
                            left={PAGE_PADDING}
                            top={0}
                            width={pdfPageWidth - PAGE_PADDING * 2}
                            height={pdfPageHeight}
                            columns={columns}
                            numbered
                          />
                        )}

                        {/* Row guides for the active page */}
                        {pageRowGuides.map((y, i) => (
                          <div
                            key={`row-guide-${i}`}
                            className="dz-row-guide"
                            style={{ top: y, width: pdfPageWidth }}
                          />
                        ))}

                        {pageEls.map((field) => {
                          const fieldLayout = layoutFor(field);
                          return fieldLayout ? (
                            <LayoutContainer
                              key={field.id ?? field.name}
                              field={field}
                              layout={fieldLayout}
                              locale={locale}
                              selected={isActive && selectionSet.has(field.name)}
                              primary={isActive && field.name === primaryName}
                              variables={variables}
                              editing={isActive && editingName === field.name}
                              onStartEdit={() => isActive && setEditingName(field.name)}
                              onCommitEdit={() => setEditingName(null)}
                            />
                          ) : null;
                        })}

                        {!isActive && (
                          <div
                            className="dz-pdf-page-overlay"
                            title={t("designer.clickToEdit")}
                          />
                        )}
                        {isActive && marquee && (
                          <div
                            className="dz-marquee"
                            style={{
                              transform: `translate3d(${marquee.x}px, ${marquee.y}px, 0)`,
                              width: marquee.width,
                              height: marquee.height,
                            }}
                          />
                        )}
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            );
          })}
        </div>
      </div>
    );
  }

  return (
    <div
      ref={viewportRef}
      className="dz-canvas2"
      onPointerDown={onBackgroundPointerDown}
      onPointerMove={onBackgroundPointerMove}
      onPointerUp={endBackgroundPointer}
      onPointerCancel={endBackgroundPointer}
      onDragOver={onDragOver}
      onDrop={onDrop}
    >
      <div className="dz-stage">
        {/* Dimension line above the form (only on a specific breakpoint, not the
            "All" base): a ruler spanning the form's width, labelled with that
            breakpoint's screen width — e.g. Mobile → 390px, SM → 640px. */}
        {activeBreakpoint !== "base" && (
          <div className="dz-dim" style={{ width: targetWidth }}>
            <span className="dz-dim-label">
              {breakpointWidth(activeBreakpoint, baseWidth)}px and up
            </span>
          </div>
        )}
        <div
          ref={pageRef}
          className={`dz-page${snap ? " has-grid" : ""}`}
          style={{
            width: pageWidth,
            height: pageHeight,
            backgroundSize: `${store.getState().gridSize}px ${store.getState().gridSize}px`,
          }}
        >
          {elements.length === 0 && (
            <p className="dz-page-empty">{t("designer.emptyHint")}</p>
          )}
          {showColGuides && (
            <ColumnGuides
              left={PAGE_PADDING}
              top={0}
              width={targetWidth - PAGE_PADDING * 2}
              height={pageHeight}
              columns={columns}
              numbered
            />
          )}
          {groupGuides.map((g) => (
            <ColumnGuides
              key={`col-guide-${g.name}`}
              left={g.left}
              top={g.top}
              width={g.width}
              height={g.height}
              columns={columns}
              accent
            />
          ))}
          {rowGuides.map((y, i) => (
            <div
              key={`row-guide-${i}`}
              className="dz-row-guide"
              style={{ top: y, width: pageWidth }}
            />
          ))}
          {elements.map((field) => {
            const fieldLayout = layoutFor(field);
            return fieldLayout ? (
              <LayoutContainer
                key={field.id ?? field.name}
                field={field}
                layout={fieldLayout}
                locale={locale}
                selected={selectionSet.has(field.name)}
                primary={field.name === primaryName}
                variables={variables}
                editing={editingName === field.name}
                onStartEdit={() => setEditingName(field.name)}
                onCommitEdit={() => setEditingName(null)}
              />
            ) : null;
          })}
          {!isDocMode && titleLayout && (
            <TitleContainer
              layout={titleLayout}
              title={resolveText(titleText, locale)}
              style={titleBox}
              locale={locale}
              selected={selectionSet.has(TITLE_NAME)}
              primary={primaryName === TITLE_NAME}
            />
          )}
          {!isDocMode && submitLayout && submittable !== false && (
            <SubmitContainer
              layout={submitLayout}
              label={resolveText(submit?.label, locale) || undefined}
              locale={locale}
              selected={selectionSet.has(SUBMIT_NAME)}
              primary={primaryName === SUBMIT_NAME}
            />
          )}
          {alignGuides.map((g, i) =>
            g.axis === "x" ? (
              <div
                key={`ag-x-${i}`}
                className="dz-align-guide dz-align-guide-x"
                style={{ left: g.position, height: pageHeight }}
              />
            ) : (
              <div
                key={`ag-y-${i}`}
                className="dz-align-guide dz-align-guide-y"
                style={{ top: g.position, width: pageWidth }}
              />
            ),
          )}
          {marquee && (
            <div
              className="dz-marquee"
              style={{
                transform: `translate3d(${marquee.x}px, ${marquee.y}px, 0)`,
                width: marquee.width,
                height: marquee.height,
              }}
            />
          )}
        </div>
      </div>
      <TextFormatToolbar />
    </div>
  );
}

// A column grid overlay: `columns` equally spaced guide lines across [left,
// left+width], optionally with a 1..columns ruler along the top. Drawn behind the
// field widgets (pointer-events: none) as the visual reference for column-based
// widths. `accent` marks a section's own grid so it reads distinctly from the
// form grid drawn underneath it.
function ColumnGuides({
  left,
  top,
  width,
  height,
  columns,
  numbered,
  accent,
}: {
  left: number;
  top: number;
  width: number;
  height: number;
  columns: number;
  numbered?: boolean;
  accent?: boolean;
}) {
  if (width <= 0 || height <= 0 || columns < 1) return null;
  const colW = width / columns;
  return (
    <div
      className={`dz-col-guides${accent ? " is-accent" : ""}`}
      style={{ left, top, width, height }}
      aria-hidden="true"
    >
      {Array.from({ length: columns + 1 }, (_, i) => (
        <span key={i} className="dz-col-line" style={{ left: i * colW }} />
      ))}
      {numbered &&
        Array.from({ length: columns }, (_, i) => (
          <span
            key={`n${i}`}
            className="dz-col-num"
            style={{ left: i * colW, width: colW }}
          >
            {i + 1}
          </span>
        ))}
    </div>
  );
}
