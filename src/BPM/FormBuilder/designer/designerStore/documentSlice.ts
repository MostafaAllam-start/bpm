...
export function createDocumentSlice(
  ...
  return {
    ...
    addField: (type, defaultTitle, at) => {
      ...
      if (!def) return;
      const { schema, snap, activePageIndex: pi } = get();
      ...
      // In PDF mode the page dimensions come from pageWidth; fall back to canvas.width.
      const canvasWidth = schema.canvas?.pageWidth ?? schema.canvas?.width ?? DEFAULT_CANVAS_WIDTH;
      ...
      commit({
        ...
        pages: schema.pages.map((p, i) =>
          i === pi ? { ...page, elements: [...shifted, field] } : p,
        ),
        submit,
      });
      set({ selection: [name], _coalesceTag: null });
    ...
    removeField: (name) => {
      ...
      const { schema, selection, activePageIndex: pi } = get();
      commit(
        mapElements(schema, (els) =>
    ...
    duplicateSelected: () => {
      const { schema, selection, gridSize, activePageIndex: pi } = get();
      if (selection.length === 0) return;
      const elements = firstPage(schema, pi).elements;
      ...
      commit(mapElements(schema, (els) => [...els, ...copies], pi));
      set({ selection: newNames, _coalesceTag: null });
    },

    moveField: (from, to) => {
      const { schema, activePageIndex: pi } = get();
      commit(
        mapElements(schema, (els) => {
          if (
            from === to ||
    ...
    setCanvasSize: (size) => {
      ...
      const canvas: CanvasSettings = {
        ...
        pageSize: size.pageSize ?? schema.canvas?.pageSize,
        pageWidth: size.pageWidth ?? schema.canvas?.pageWidth,
        pageHeight: size.pageHeight ?? schema.canvas?.pageHeight,
      };
    ...
    addPage: () => {
      const { schema } = get();
      const n = schema.pages.length + 1;
      const newPage = { name: `page${n}`, elements: [] };
      const next = { ...schema, pages: [...schema.pages, newPage] };
      set({ schema: next, activePageIndex: schema.pages.length });
    },

    removePage: (index) => {
      const { schema, activePageIndex } = get();
      if (schema.pages.length <= 1) return;
      const pages = schema.pages.filter((_, i) => i !== index);
      const next = { ...schema, pages };
      const nextActive = activePageIndex >= pages.length ? pages.length - 1 : activePageIndex;
      set({ schema: next, activePageIndex: nextActive, selection: [] });
    },

    setActivePageIndex: (index) => {
      const { schema } = get();
      const clamped = Math.max(0, Math.min(index, schema.pages.length - 1));
      set({ activePageIndex: clamped, selection: [] });
    },
  };
}
