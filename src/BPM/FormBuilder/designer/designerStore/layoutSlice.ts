...
import { computeAlignGuides } from "../canvasAlignGuides";
...
export function createLayoutSlice(
  ...
  return {
    updateLayout: (name, patch) => {
      ...
      if (!cur) return;
      const fieldType = schema.pages[pi]?.elements.find((e) => e.name === name)?.type;
    ...
    moveSelectedTo: (primary, pos) => {
      ...
      const pi = get().activePageIndex;
      const baseEls = firstPage(baseSchema, pi).elements;
    ...
    resizeField: (name, box) => {
      const { schema, snap, gridSize, _baseline, activeBreakpoint, activePageIndex: _pi } = get();
      const fieldType = schema.pages[_pi]?.elements.find((e) => e.name === name)?.type;
      const minH = fieldType === "divider" ? MIN_DIVIDER_HEIGHT : MIN_HEIGHT;
      ...
      const pi = get().activePageIndex;
      if (firstPage(schema, pi).elements.find((e) => e.name === name)?.type === "group") {
        live(
      ...
      const baseFields = firstPage(base, pi).elements;
      ...
      live({
        ...
        pages: schema.pages.map((p, i) =>
          i === pi ? { ...page, elements: normalizeGroups(elements, schema.canvas?.width) } : p,
        ),
        submit,
      });
    },
