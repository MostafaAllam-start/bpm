...
export default function PropertyPanel({
  ...
  return (
    <aside className="dz-props">
      ...
      {has("title") && !(field.type === "group" && isDocMode) && (
        <LocalizedField
          label={t("designer.props.title")}
          value={field.title}
          variables={field.type === "heading" ? undefined : varsFor(field.name)}
          renderHtml={field.type === "heading"}
      ...
      {has("backgroundImage") && (
        <SectionBgEditor field={field} patch={patch} />
      )}
...
// A contentEditable div that displays and edits rich HTML. Commits on blur.
// Used for heading title rows where the stored value is HTML, not plain text.
function HtmlEditableField({
  ...
  // Set innerHTML on mount only; subsequent edits are driven by the user inside
  // the contentEditable — we must not overwrite the DOM while they type.
  useEffect(() => {
    if (ref.current) {
  ...
  return (
    <div
      ref={ref}
      className="dz-prop-html-preview"
      contentEditable
...
function LocalizedField({
  ...
  hint?: string;
  variables?: VariableSet;
  // When true the stored value is rich HTML; show a rendered preview instead of
  // a plain text input. Editing happens via the canvas InlineEditor.
  renderHtml?: boolean;
        ...
        <div key={lng} className="dz-loc-row">
          ...
          {renderHtml ? (
            <HtmlEditableField
              ...
              onChange={(html) => setLocale(lng, html)}
            />
          ) : variables ? (
            <MentionField
              value={getLocaleText(value, lng)}
...
function SectionBgEditor({
  ...
  const { t } = useTranslation("form");
  const bg = field.backgroundImage ?? "";
  const isUpload = bg.startsWith("data:");
  ...
  const readFile = (file: File) => {
    if (!file.type.startsWith("image/")) return;
    const reader = new FileReader();
    reader.onload = () => patch({ backgroundImage: String(reader.result) || undefined });
    reader.readAsDataURL(file);
  };

  return (
    <div className="dz-section-group">
      <div className="dz-section-head">{t("designer.props.background")}</div>

      <div className="dz-prop">
        <span className="dz-prop-label">{t("designer.props.backgroundImage")}</span>
        ...
        {inputMode === "url" ? (
          ...
          <div className="dz-image-upload">
            <Dropzone
              ...
              onFile={readFile}
            >
              <span className="ff-dropzone-icon" aria-hidden="true">⬆</span>
              <span className="ff-dropzone-text">{t("designer.image.dropzone")}</span>
              <span className="ff-dropzone-hint">{t("designer.image.dropzoneHint")}</span>
            </Dropzone>
          </div>
        )}
      ...
      {bg && (
        <>
          <label className="dz-prop">
            <span className="dz-prop-label">{t("designer.props.backgroundRepeat")}</span>
            <select
              className="dz-prop-input"
              value={field.backgroundRepeat ?? "no-repeat"}
              onChange={(e) =>
                patch({ backgroundRepeat: e.target.value as FormField["backgroundRepeat"] })
              }
            >
              {(["no-repeat", "repeat", "repeat-x", "repeat-y", "space", "round"] as const).map(
          ...
          <label className="dz-prop">
            <span className="dz-prop-label">{t("designer.props.backgroundSize")}</span>
            <select
              className="dz-prop-input"
              value={
                ["cover", "contain", "auto"].includes(field.backgroundSize ?? "")
                  ? (field.backgroundSize ?? "cover")
                  : "custom"
              }
              onChange={(e) => {
                if (e.target.value !== "custom") patch({ backgroundSize: e.target.value });
              }}
            >
              <option value="cover">cover</option>
              <option value="contain">contain</option>
              <option value="auto">auto</option>
              <option value="custom">{t("designer.canvas.custom")}</option>
            </select>
            {!["cover", "contain", "auto"].includes(field.backgroundSize ?? "") && (
              <input
                type="text"
                className="dz-prop-input"
                placeholder="100% 100%"
                value={field.backgroundSize ?? ""}
                onChange={(e) => patch({ backgroundSize: e.target.value || undefined })}
              />
            )}
          </label>

          <label className="dz-prop">
            <span className="dz-prop-label">{t("designer.props.backgroundPosition")}</span>
            <input
              type="text"
              className="dz-prop-input"
              placeholder="center center"
              value={field.backgroundPosition ?? ""}
              onChange={(e) => patch({ backgroundPosition: e.target.value || undefined })}
            />
          </label>

          <label className="dz-prop">
            <span className="dz-prop-label">{t("designer.props.backgroundAttachment")}</span>
            <select
              className="dz-prop-input"
              value={field.backgroundAttachment ?? "scroll"}
              onChange={(e) =>
                patch({ backgroundAttachment: e.target.value as FormField["backgroundAttachment"] })
              }
            >
              {(["scroll", "fixed", "local"] as const).map((v) => (
