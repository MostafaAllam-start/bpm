...
export function TableField(p: FieldRenderProps) {
  ...
  // the inline cell editor); API cells are plain text from the endpoint.
  type BodyRow = { html: boolean; cells: string[] };
  // Turn a manual row set into resolved cells, aligned to the columns.
  const manual = (rows: LocalizedText[][] | undefined): BodyRow[] =>
    (rows ?? []).map((row) => ({
      html: true,
  ...
  const bodyRows: BodyRow[] = isApi
    ? [
        ...manual(field.tableTopRows),
        ...apiRows.map((cells) => ({ html: false, cells })),
        ...manual(field.tableBottomRows),
      ]
          ...
          <thead>
            <tr>
              {headers.map((h, i) => (
                ...
                <th key={i} dir="auto" dangerouslySetInnerHTML={{ __html: h }} />
              ))}
            </tr>
          </thead>
        )}
