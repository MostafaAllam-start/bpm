  ...
  const onNodeContextMenu = useCallback(
    (event: React.MouseEvent, node: BpmnNode) => {
      ...
      if (spec.actor) {
        ...
        const hasPdf = Boolean(savedPdfReports?.[node.id]);
        if (!isStart) {
          items.push({
        ...
        if (node.data.bpmnType === "userTask") {
          items.push({
            labelKey: hasPdf ? "contextMenu.updatePdfReport" : "contextMenu.addPdfReport",
            icon: "pdf",
            onClick: () => { onOpenPdfDesigner?.(node.id, label); closeMenu(); },
          });
        }
        items.push("separator");
      }
      if (node.data.bpmnType === "sendEmailTask") {
        const label = node.data.name || node.id;
        const hasEmail = Boolean(savedEmailDesigns?.[node.id]);
        items.push(
          {
            labelKey: hasEmail ? "contextMenu.updateEmail" : "contextMenu.addEmail",
            icon: "email",
            onClick: () => { onOpenEmailDesigner?.(node.id, label); closeMenu(); },
          },
          "separator",
        );
      }
        ...
        <div className="bpmn-body" dir="ltr">
          ...
          {simActive ? (
            ...
            <PropertiesPanel
              ...
              onEditEmailDesign={onOpenEmailDesigner ? (id, label) => { onOpenEmailDesigner(id, label); } : undefined}
              savedPdfReports={savedPdfReports ?? {}}
              onEditPdfReport={onOpenPdfDesigner ? (id, label) => { onOpenPdfDesigner(id, label); } : undefined}
            />
          )}
        </div>
