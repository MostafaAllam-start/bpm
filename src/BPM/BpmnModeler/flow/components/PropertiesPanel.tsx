...
type PropsControlsProps = {
  ...
  // Email designer — available on all actor tasks (excluding start event).
  savedEmailDesigns?: Record<string, SavedActorForm>;
  onEditEmailDesign?: (nodeId: string, label: string) => void;
  // PDF report designer — available on userTask only.
  savedPdfReports?: Record<string, SavedActorForm>;
  onEditPdfReport?: (nodeId: string, label: string) => void;
};
  ...
  if (selectedNode) {
    ...
    return (
      <div className="bf-properties">
        ...
        {data.bpmnType === "sendEmailTask" && (() => {
          const hasEmail = Boolean(savedEmailDesigns?.[selectedNode.id]);
          const label = data.name || selectedNode.id;
          return (
            <>
              <div className="bf-prop-subtitle">{t("props.emailDesign")}</div>
              <div className="bf-var-hint">{t("props.emailDesignHint")}</div>
              {!hasEmail && <div className="bf-actor-empty">{t("props.noEmailDesign")}</div>}
              <button
                type="button"
                className="bf-var-add"
                disabled={!onEditEmailDesign}
                onClick={() => onEditEmailDesign?.(selectedNode.id, label)}
              >
                + {hasEmail ? t("props.updateEmailDesign") : t("props.addEmailDesign")}
              </button>
            </>
          );
        })()}

        {data.bpmnType === "userTask" && (() => {
          const hasPdf = Boolean(savedPdfReports?.[selectedNode.id]);
          const label = data.name || selectedNode.id;
          return (
            <>
              <div className="bf-prop-subtitle">{t("props.pdfReport")}</div>
              <div className="bf-var-hint">{t("props.pdfReportHint")}</div>
              {!hasPdf && <div className="bf-actor-empty">{t("props.noPdfReport")}</div>}
              <button
                type="button"
                className="bf-var-add"
                disabled={!onEditPdfReport}
                onClick={() => onEditPdfReport?.(selectedNode.id, label)}
              >
                + {hasPdf ? t("props.updatePdfReport") : t("props.addPdfReport")}
              </button>
            </>
          );
        })()}
