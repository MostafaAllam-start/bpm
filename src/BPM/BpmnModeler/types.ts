...
export type BpmnEditorProps = {
  ...
  // Email designer — one template per task node (any actor task).
  savedEmailDesigns?: Record<string, SavedActorForm>;
  onOpenEmailDesigner?: (nodeId: string, nodeLabel: string) => void;

  // PDF report designer — one report per userTask node.
  savedPdfReports?: Record<string, SavedActorForm>;
  onOpenPdfDesigner?: (nodeId: string, nodeLabel: string) => void;
};
